...
use crate::lib::encrypt::{decrypt, encrypt, new_key};
use crate::models::*;
...
pub fn verify_db() -> bool {
    ...
    let mut stmt = conn
        ...
        .map_err(|err| format!("the error is {}", err.to_string()))
        .unwrap();
...
pub fn create_user(user: User) -> bool {
    ...
    let _ = conn.execute(
        "INSERT INTO user (id, name, password, image) VALUES (?1, ?2, ?3, ?4)",
...
pub fn get_image_user() -> String {
    ...
    let mut stmt = conn
        .prepare("SELECT image FROM user")
        .map_err(|err| format!("the error is {}", err.to_string()))
        .unwrap();
...
pub fn login(password: String) -> bool {
    ...
    let mut stmt = conn
        .prepare("SELECT password FROM user")
        .map_err(|err| format!("the error is {}", err.to_string()))
        .unwrap();
...
pub fn get_passwords() -> Vec<Password> {
    ...
    let mut stmt = conn
        .prepare(
            "SELECT id, name, icon, user, user_length, password, password_length FROM password",
        )
        .map_err(|err| format!("the error is {}", err.to_string()))
...
pub fn delete_password(id: i32) {
    ...
    let _ = conn
        .execute("DELETE FROM password WHERE id = ?1", params![id])
        .expect("error while deleting password");
}
...
pub fn update_user_without_password(user: User) {
    ...
    let _ = conn
        .execute(
            "UPDATE user SET name = ?1, image = ?2",
            params![user.name, user.image],
        )
...
pub fn update_user_with_password(user: User, password_now: String) {
    ...
    let _ = conn
        .execute(
            "UPDATE user SET name = ?1, password = ?2, image = ?3",
            params![user.name, hashed_password, user.image],
        )
        .expect("error while updating user");
    ...
    for password_array in passwords {
        ...
        let _ = conn
            .execute(
                "UPDATE password SET user = ?1, password = ?2 WHERE id = ?3",
                params![user, password, password_array.id],
            )
            .expect("error while updating password");
    }
}
